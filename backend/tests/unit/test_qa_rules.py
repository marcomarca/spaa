...
def test_qa_fails_on_silence(tmp_path: Path):
    ...
    res = AudioQARules.evaluate_chunk_audio(
        valid_size_file, word_count=100, duration_seconds=30.0, is_silent=True
    )
    assert not res.passed
...
def test_qa_passes_on_valid_proportions(tmp_path: Path):
    ...
    res = AudioQARules.evaluate_chunk_audio(
        valid_file, word_count=150, duration_seconds=60.0, is_silent=False
    )
    assert res.passed
