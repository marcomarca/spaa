...
class Qwen3TTSModel:
    ...
    def from_pretrained(
        ...
        **kwargs,
        ...
        """
        # Register model types with AutoClasses
        # These may fail in older transformers versions
        try:
            AutoConfig.register("qwen3_tts", Qwen3TTSConfig)
        except Exception as e:
            print(f"Warning: AutoConfig.register failed: {e}")
        
        try:
            AutoModel.register(Qwen3TTSConfig, Qwen3TTSForConditionalGeneration)
        except Exception as e:
            print(f"Warning: AutoModel.register failed: {e}")
        
        try:
            AutoProcessor.register(Qwen3TTSConfig, Qwen3TTSProcessor)
        except Exception as e:
            print(f"Warning: AutoProcessor.register failed: {e}")

        # Load model - try AutoModel first, fall back to direct loading
        try:
            model = AutoModel.from_pretrained(pretrained_model_name_or_path, **kwargs)
        except Exception as e:
            print(f"AutoModel.from_pretrained failed: {e}, trying direct load...")
        ...
        # Try AutoProcessor first, fall back to manual loading
        try:
            processor = AutoProcessor.from_pretrained(pretrained_model_name_or_path, fix_mistral_regex=True,)
        except Exception as e:
            print(f"AutoProcessor.from_pretrained failed: {e}, trying manual load...")
            # Manual processor loading for older transformers
